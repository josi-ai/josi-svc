...
class AIInterpretationService:
    ...
    def __init__(self):
        ...
        self.qdrant_client = QdrantClient(url=settings.qdrant_url) if settings.qdrant_url else None
    ...
    def _init_vector_collection(self):
        """Initialize Qdrant collection for storing interpretations."""
        try:
            collections = self.qdrant_client.get_collections().collections
            if not any(c.name == "astrology_interpretations" for c in collections):
                self.qdrant_client.create_collection(
                    collection_name="astrology_interpretations",
                    vectors_config=VectorParams(size=384, distance=Distance.COSINE),
                )
                logger.info("Created Qdrant collection for interpretations")
    ...
    async def generate_interpretation(
        ...
        try:
            # Search for similar interpretations if vector DB is available
            ...
            if self.qdrant_client:
                await self._store_interpretation(chart_data, question, response_text)
...
Please provide a detailed, insightful interpretation addressing the user's question. Be specific to this chart and avoid generic statements."""
    ...
    async def _search_similar_interpretations(
        ...
        """Search for similar interpretations in vector database."""
        ...
        try:
            ...
            embedding = self.encoder.encode(search_text)
                ...
                if result.score > 0.7:  # Similarity threshold
                    similar.append({
                        ...
                        "key_insight": self._extract_key_insight(
                            result.payload.get("interpretation", "")
                        ),
                        "chart_similarity": result.payload.get("chart_features", {})
                    })
    ...
    async def _store_interpretation(
        ...
        """Store interpretation in vector database for future reference."""
        if not self.qdrant_client:
            return
        ...
        try:
            ...
            point_id = f"{chart_hash}_{question_hash}"
            
            # Create embedding
            text = f"{question} {interpretation}"
